...
const DEFAULT_PRODUCTS=[
  ...
  {id:'aerolight',name:'Stanley Aerolight',tier:'Atractivo',quota:15,stock:8,sort:2,img:'https://raw.githubusercontent.com/cristopherlolimendoza-coder/HONOR-Rewards/5b4984d4aa17b5c820f859ee37664646365db55b/assets/stanley-aerolight-hires.webp',desc:'Botella ligera Stanley para campañas recurrentes.'},
...
async function isAdmin(req,env){
  try{
    ...
    const m=cookie.match(/(?:^|;\s*)hr_admin=([^;]+)/);
    if(!m)return false;
...
async function createRedeem(req,db){
  ...
  for(let i=0;i<clients.length;i++){
    const c=clients[i]||{};
    const ruc=clean(c.ruc),company=clean(c.company),models=Array.isArray(c.models)?c.models:[];
    ...
    for(const m of models){
      ...
      total+=qty;
      lines.push({ruc,company,model,qty});
    }
  }
  ...
  try{
    ...
    for(const l of lines){
      await db.prepare(`INSERT INTO request_lines(request_id,ruc,company,model,qty) VALUES(?,?,?,?,?)`)
        .bind(id,l.ruc,l.company,l.model,l.qty).run();
    }
  }catch(e){
    await db.prepare(`UPDATE products SET available=available+1,reserved=CASE WHEN reserved>0 THEN reserved-1 ELSE 0 END WHERE id=?`).bind(productId).run();
    throw e;
  }
...
async function adminRequests(db){
  ...
  for(const r of (results||[])){
    const q=await db.prepare(`SELECT ruc,company,model,qty FROM request_lines WHERE request_id=? ORDER BY id`).bind(r.id).all();
    r.lines=q.results||[];
  }
  return results||[];
}
...
async function updateStatus(req,db){
  ...
  if(!b)return json({error:'Datos inválidos.'},400);
  ...
  const old=r.status;
  if(old===next){
    await db.prepare(`UPDATE requests SET admin_note=? WHERE id=?`).bind(note,id).run();
    return json({ok:true,status:next});
  }
  if((old==='En revisión'||old==='Aprobado')&&next==='Rechazado'){
    await db.prepare(`UPDATE products SET reserved=CASE WHEN reserved>0 THEN reserved-1 ELSE 0 END,available=available+1 WHERE id=?`).bind(r.product_id).run();
  }else if((old==='En revisión'||old==='Aprobado')&&next==='Entregado'){
    await db.prepare(`UPDATE products SET reserved=CASE WHEN reserved>0 THEN reserved-1 ELSE 0 END,delivered=delivered+1 WHERE id=?`).bind(r.product_id).run();
  }else if(old==='En revisión'&&next==='Aprobado'){
    // El stock permanece reservado.
  }else{
    return json({error:'Cambio de estado no permitido.'},400);
  }
...
async function updateProduct(req,db){
  ...
  if(!p)return json({error:'Premio no encontrado.'},404);
...
export async function onRequest(context){
  ...
  if(path==='/api/health'&&request.method==='GET'){
    return json({ok:true,databaseConfigured:!!env.DB,adminPasswordConfigured:!!env.ADMIN_PASSWORD,sessionSecretConfigured:!!env.SESSION_SECRET});
  }
  ...
  try{
    ...
    return json({error:'No se pudo inicializar la base de datos.',detail:String(e?.message||e)},500);
  }
  ...
  try{
    ...
    if(path==='/api/admin/logout'&&request.method==='POST'){
      return json({ok:true},200,{'set-cookie':'hr_admin=; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=0'});
    }
    ...
    return json({error:'Error interno.',detail:String(e?.message||e)},500);
  }
}
